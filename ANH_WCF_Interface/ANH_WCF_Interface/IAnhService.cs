using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.Serialization;
using System.ServiceModel;
using System.Text;

namespace ANH_WCF_Interface
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the interface name "IService1" in both code and config file together.
    [ServiceContract(CallbackContract = typeof(IAnhCallback), SessionMode = SessionMode.Required)]
    public interface IAnhService
    {
        [OperationContract]
        Boolean AuthenticateSession(String hash);

        [OperationContract]
        String StartServer(ServerType type, String args);

        [OperationContract]
        String StopServer(ServerType type, String args);

        [OperationContract]
        List<IServerStatus> GetServerStatuses();
        
        [OperationContract]
        List<ServerType> GetAvailableServers();

        [OperationContract]
        String SubscribeToStatusUpdates();

        [OperationContract]
        String UnsubscribeFromStatusUpdates();
    }
}
